...
- **Auto AI notes**: periodically sends the video's transcript (captions) to Claude, which extracts concise key points with real timestamps — click a timestamp to seek the video.
...
- All processing is local; the only network call is your own request to the Anthropic API using your own key.

## Why bring your own API key

There's no backend server here — that's what keeps this free and keeps your notes private. AI note-taking calls the Claude API directly from your browser using an API key you provide (Settings → paste your key). This means:

- No subscription, no server costs to you beyond Anthropic's own token pricing (typically a small fraction of a cent per video with Claude Sonnet, and the Haiku model is cheaper still).
- Your key and your video transcripts never pass through any third party — the request goes straight from your browser to `api.anthropic.com`.
...
4. Open a YouTube video, click the floating 📝 button (bottom-right), then the ⚙ gear to add your Anthropic API key.
...
- `background/background.js` is the only place that calls the Anthropic API, using the key you saved in Settings.
...
- No analytics, no telemetry, no third-party server. The only outbound request AI notes make is your browser → `api.anthropic.com`, authenticated with the key you provide.
